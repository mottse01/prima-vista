...
function validate(fragment) {
  ...
  for (let bar = 0; bar < fragment.bars; bar++) {
    ...
    if (Math.abs(duration - 4) > 0.0001) throw new Error(`Invalid fragment duration: ${fragment.id}, bar ${bar + 1}`);
  }
...
export function chooseFragment(rng, { meter, level }) {
  const eligible = FRAGMENTS.filter((fragment) => fragment.meter === meter && fragment.level <= level);
